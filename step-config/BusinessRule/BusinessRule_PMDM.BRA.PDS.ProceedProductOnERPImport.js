...
{
  "id" : "PMDM.BRA.PDS.ProceedProductOnERPImport",
  "type" : "BusinessAction",
  "setupGroups" : [ "PMDM.BusinessRuleActions" ],
  "name" : "Proceed Product on ERP Import",
  "description" : null,
  "scope" : "Global",
...
*/
exports.operation0 = function (node,logger,erpStatusCodeAttribute,internalSourceRecordObjectType,manager,importChangeInfo,logLib) {
var forceLog = false;

function log(message) {
	logLib.log(logger, "Proceed Product on ERP Import: " + message, forceLog);
}

function initiateInINTCreationWorkflow(node) {
	var intCreationWorkflowInstance = node.startWorkflowByID("PMDM.WF.InternalSourceRecordCreation", msg1); // "Started"
	log(node.getID() + " has been started in Internal Source Record Creation workflow");
}

function initiateInERPCommunicationWorkflow(node) {
	var erpCommunicationWorkflowInstance = node.startWorkflowByID("PMDM.WF.ERPCommunication", msg1); // "Started"
	log(node.getID() + " has been started in ERP Communication workflow");
	var result = null;
...
if (nodeObjectTypeID.equals(internalSourceRecordObjectType.getID())) {
	...
	// If INT is created by current import then initiate it in INT Creation workflow
	if (importChangeInfo.isCreatedByCurrentProcess()) {
		initiateInINTCreationWorkflow(node);
	}
	...
	if (!node.isInWorkflow(erpCommunicationWorkflowID)) {
		if (erpStatusCode != null && erpStatusCode != "0") {
			var result = initiateInERPCommunicationWorkflow(node);
			log("Error [" + node.getTitle() + "] result=" + result);
		} else if (erpStatusCode == null) {
			var result = initiateInERPCommunicationWorkflow(node);
			log("Attribute 'ERP Status Code' Empty [" + node.getTitle() + "] result=" + result);
