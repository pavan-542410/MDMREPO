...
{
  "id" : "PMDM.BRA.UnblockPublishToERP_GR",
  "type" : "BusinessAction",
  "setupGroups" : [ "PMDM.BusinessRuleActions" ],
  "name" : "Unblock Publish To ERP (on Golden Record)",
  "description" : null,
  "scope" : "Global",
...
function log(message) {
	logLib.log(logger, "Unblock Publish To ERP: " + message, forceLog);
}

function unblockPublishToERP(node) {
	node.getValue(blockPublishToERPAttribute.getID()).setLOVValueByID("N");
}

var objectTypeID = node.getObjectType().getID();
if(goldenRecordObjectType.getID().equals(objectTypeID)) {
	unblockPublishToERP(node);
	...
	if (goldenRecord) {
		unblockPublishToERP(goldenRecord);
	}	
}
