...
{
  "id" : "PMDM.BRA.BlockPublishToERP_GR",
  "type" : "BusinessAction",
  "setupGroups" : [ "PMDM.BusinessRuleActions" ],
  "name" : "Block Publish To ERP (on Golden Record)",
  "description" : null,
  "scope" : "Global",
...
function log(message) {
	logLib.log(logger, "Block Publish To ERP: " + message, forceLog);
}

function blockPublishToERP(node) {
	node.getValue(blockPublishToERPAttribute.getID()).setLOVValueByID("Y");	
}

var objectTypeID = node.getObjectType().getID();
if(goldenRecordObjectType.getID().equals(objectTypeID)) {
	blockPublishToERP(node);
	...
	if (goldenRecord) {
		blockPublishToERP(goldenRecord);
	}	
}
