...
{
  "id" : "PMDM.BRA.BlockPublishToEcommerce_GR",
  "type" : "BusinessAction",
  "setupGroups" : [ "PMDM.BusinessRuleActions" ],
  "name" : "Block Publish To Ecommerce (on Golden Record)",
  "description" : null,
  "scope" : "Global",
...
function log(message) {
	logLib.log(logger, "Block Publish To Ecommerce: " + message, forceLog);
}

function blockPublishToEcommerce(node) {
	node.getValue(blockPublishToEcommerceAttribute.getID()).setLOVValueByID("Y");	
}

var objectTypeID = node.getObjectType().getID();
if(goldenRecordObjectType.getID().equals(objectTypeID)) {
	blockPublishToEcommerce(node);
	...
	if (goldenRecord) {
		blockPublishToEcommerce(goldenRecord);
	}	
}
